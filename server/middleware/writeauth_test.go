...
func doLegacyRequest(r *gin.Engine, token, actingUserSub string) *httptest.ResponseRecorder {
...
func TestWriteAuth_LegacyFallback_RejectsWhenTokenUnset(t *testing.T) {
	t.Setenv("INTERNAL_SERVICE_TOKEN", "")
	r := newWriteAuthTestRouter("")

	rec := doLegacyRequest(r, "anything", "user-sub")
	if rec.Code != http.StatusUnauthorized {
		t.Errorf("status = %d, want %d", rec.Code, http.StatusUnauthorized)
	}
}

func TestWriteAuth_LegacyFallback_RejectsInvalidToken(t *testing.T) {
	t.Setenv("INTERNAL_SERVICE_TOKEN", "secret")
	r := newWriteAuthTestRouter("")

	rec := doLegacyRequest(r, "wrong-token", "user-sub")
	if rec.Code != http.StatusUnauthorized {
		t.Errorf("status = %d, want %d", rec.Code, http.StatusUnauthorized)
	}
}

func TestWriteAuth_LegacyFallback_RejectsMissingActingUserSub(t *testing.T) {
	t.Setenv("INTERNAL_SERVICE_TOKEN", "secret")
	r := newWriteAuthTestRouter("")

	rec := doLegacyRequest(r, "secret", "")
	if rec.Code != http.StatusUnauthorized {
		t.Errorf("status = %d, want %d", rec.Code, http.StatusUnauthorized)
	}
}

func TestWriteAuth_LegacyFallback_AcceptsValidCombination(t *testing.T) {
	t.Setenv("INTERNAL_SERVICE_TOKEN", "secret")
	r := newWriteAuthTestRouter("")

	rec := doLegacyRequest(r, "secret", "auth0|user-sub")
	if rec.Code != http.StatusOK {
		t.Errorf("status = %d, want %d", rec.Code, http.StatusOK)
	}
	if got := rec.Body.String(); got != `{"actingUserSub":"auth0|user-sub"}` {
