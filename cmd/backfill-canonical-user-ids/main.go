// Command backfill-canonical-user-ids rewrites legacy Auth0-subject values in the
// banners and maintenance_modes audit fields (created_by, updated_by) to the canonical
...
// write. Run once after the admin-api release that adopts authz.Viewer.
package main
...
var targets = []target{
	{"banners", "created_by"},
	{"banners", "updated_by"},
	{"maintenance_modes", "created_by"},
	{"maintenance_modes", "updated_by"},
}
...
func main() {
	...
	if *apply {
		...
		fmt.Println("dry run complete - pass -apply to write changes; still remaining check via task 5.3")
	}
}
