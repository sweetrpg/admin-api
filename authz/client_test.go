...
func TestCheckAllowed(t *testing.T) {
	...
	resp, err := client.Check(context.Background(), "good-token", "admin-api")
	if err != nil {
...
func TestCheckInvalidToken(t *testing.T) {
	...
	client := NewClient(srv.URL)
	_, err := client.Check(context.Background(), "bad-token", "admin-api")
...
func TestCheckDenied(t *testing.T) {
	...
	client := NewClient(srv.URL)
	resp, err := client.Check(context.Background(), "denied-token", "admin-api")
	if err != nil {
		t.Fatalf("Check() error = %v, want nil", err)
	}
	if resp.Allowed {
...
func TestCheckBackendUnreachable(t *testing.T) {
	...
	_, err := client.Check(context.Background(), "any-token", "admin-api")
	if err == nil {
